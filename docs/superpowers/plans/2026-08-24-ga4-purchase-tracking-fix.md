...
**Goal:** Make the GA4 `purchase` event reliably reach `gtag()` on checkout return from Stripe, instead of racing an in-memory queue that a same-tick `window.location.reload()` destroys.

**Architecture:** Extract the purchase-params construction into a pure function (`public/js/purchase-tracking.js`), call it directly from `gtag()` in `handleCheckoutResult()` — bypassing `analyticsManager`'s queue entirely for this one event — with a `getUserPlan(user.uid)` fallback when `sessionStorage.pendingCheckoutPlan` is missing. A small testability seam (`window.__reloadPage`) lets a Playwright harness prove the event fires before the reload, without needing to fully boot the real authenticated app.
...
Run: `node public/.test/test-purchase-tracking.js`
Expected: `RESULTADO: 13 pasaron, 0 fallaron` (exit code 0)
...
```js
        ...
        if (!purchaseParams._planFound && typeof window.getUserPlan === 'function') {
            try {
                ...
                if (confirmedPlan && confirmedPlan.id) {
                    purchaseParams = window.buildPurchaseParams(sessionId, confirmedPlan.id, window.PLANS);
                }
            } catch (e) {
                debugLog('[STRIPE] No se pudo confirmar plan via getUserPlan para el evento purchase:', e.message);
            }
        }
        const purchaseValue = purchaseParams.value;

        // GA4 — directo a gtag, sin pasar por analyticsManager: su cola en
        // memoria no sobrevive el reload() de un par de lineas mas abajo.
        // gtag.js ya usa sendBeacon internamente, que si sobrevive.
...
<script>
  ...
  window.gtag = function (...args) { window.__callLog.push(['gtag', args]); };
  window.PLANS = {
    professional: { id: 'professional', name: 'Professional', price: 14.99 },
  };
 ...
 * confirma que gtag('event','purchase',...) se llama ANTES que
 * window.__reloadPage() en el flujo de regreso de Stripe.
 *
...
(async () => {
    ...
    await page.goto('http://localhost:8090/.test/purchase-flow-harness.html?session_id=cs_test_playwright_123', {
        waitUntil: 'networkidle',
        timeout: 20000,
    });
    await page.waitForTimeout(500); // dejar correr el onAuthStateChanged asincrono

    const log = await page.evaluate(() => window.__callLog);

    console.log('\n=== purchase-flow-harness — call log ===');
    ...
    assert(gtagPurchaseIdx !== -1, 'gtag("event","purchase",...) se llamo al menos una vez');
    assert(reloadIdx !== -1, '__reloadPage() se llamo (el flujo llego hasta el final)');
    assert(gtagPurchaseIdx !== -1 && reloadIdx !== -1 && gtagPurchaseIdx < reloadIdx,
        'gtag("purchase") se dispara ANTES de __reloadPage() -- el criterio de exito de este fix');

    if (gtagPurchaseIdx !== -1) {
        const purchaseParams = log[gtagPurchaseIdx][1][2];
        assert(purchaseParams.transaction_id === 'cs_test_playwright_123', 'transaction_id coincide con el session_id de la URL');
        assert(purchaseParams.value === 14.99, 'value viene del plan real (via sessionStorage en este caso) (' + purchaseParams.value + ')');
    }
...
Expected: `RESULTADO: 6 pasaron, 0 fallaron`, and the printed call log shows
`gtag` (event purchase) appearing before `__reloadPage` in the sequence.
